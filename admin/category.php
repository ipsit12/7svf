                                    ...
                                    if(mysqli_num_rows($result) > 0) {
                                        while($row = mysqli_fetch_assoc($result)) {
                                           ...
                                           <tr>
                                            ...
                                            <td><button class="btn btn-danger" href="delcategory.php?id=<?php echo $row['id'] ?>">DELETE</button></td>
                                           </tr>
                                           <?php
                                        }
                                    } else {
...
if(isset($_POST['submit-form'])){
    $category = $_POST['category'];
        ...
        $sql_query = "INSERT INTO `category`(`category`)
                      VALUES ('$category')";

        $sql_result = mysqli_query($conn, $sql_query);

        if($sql_result){
            echo "<script>alert('Data inserted successfully'); window.location.assign('./category.php');</script>";
        } else {
